...
def process_pdf(pdf_bytes, filename):
    ...
    # Inicializa o nome como "não encontrado"
    nome = "Nome não encontrado"

    # Tentativa 1: COMERCIAL NORMAL CONVENCIONAL
    match_comercial = re.search(r'COMERCIAL NORMAL CONVENCIONAL\s+([A-Z\s\-\.\&]+)', text)
    if match_comercial:
        nome = match_comercial.group(1).strip()
    else:
        # Tentativa 2: RESIDENCIAL - RESIDENCIAL NORMAL CONVENCIONAL
