...
def extract_unidade_consumidora(text):
    match = re.search(r'UNIDADE CONSUMIDORA\s*[:\-]?\s*(\d{9,11})', text, re.IGNORECASE)
    if match:
        return match.group(1)
    return "Não encontrado"

def extract_numero_nota_fiscal(text):
    match = re.search(r'Nota Fiscal\s*(?:Eletrônica\s*)?(?:Nº|Número)?[:\-]?\s*(\d{9,12})', text, re.IGNORECASE)
    if match:
        return match.group(1)
    return "Não encontrado"
...
def process_pdf(pdf_bytes, filename):
    ...
    nf = extract_numero_nota_fiscal(text)
