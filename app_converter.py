...
def process_pdf(pdf_bytes, filename):
    ...
    # Tentativa 1: usar COMERCIAL NORMAL CONVENCIONAL como âncora
    nome = "Nome não encontrado"
    match_nome = re.search(r'COMERCIAL NORMAL CONVENCIONAL\s+([A-Z\s\-\.\&]+)', text)
    if match_nome:
        nome = match_nome.group(1).strip()
    else:
        # Tentativa 2: usar texto antes de CNPJ/CPF
